...
export default function StepCompanyBasics({
  draft,
  updateDraft,
  errors,
}: StepCompanyBasicsProps) {
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Provide up to three company name preferences. We will check
            availability and register your first available choice.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {[0, 1, 2].map((index) => (
            <div key={index} className="space-y-2">
              <Label htmlFor={`name${index + 1}`}>
                {index === 0
                  ? "First Choice *"
                  : `${index === 1 ? "Second" : "Third"} Choice (Optional)`}
              </Label>
              <Input
                id={`name${index + 1}`}
          ...
          <p className="text-sm text-muted-foreground">
            Note: "Limited" or "Ltd" will be automatically added to your company
            name.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select the type of company you want to form.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => updateDraft({ companyType: value })}
            >
              <SelectTrigger id="companyType">
                <SelectValue placeholder="Select company type" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="private-limited-shares">
                  Private Limited by Shares
                </SelectItem>
                <SelectItem value="private-limited-guarantee">
