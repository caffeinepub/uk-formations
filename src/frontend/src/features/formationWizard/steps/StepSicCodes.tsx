...
export default function StepSicCodes({ draft, updateDraft, errors }: StepSicCodesProps) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            SIC codes describe your business activities. You can add up to 4 codes. Choose the codes that best
            describe what your company will do.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {errors.sicCodes && <p className="text-sm text-destructive">{errors.sicCodes}</p>}
          ...
          <div className="flex gap-2">
            ...
            <div className="flex items-end">
              <Button onClick={addSicCode} disabled={!newCode.trim() || draft.sicCodes.length >= 4}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Click to add a common SIC code to your selection.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {commonCodes.map((item) => (
              <button
                key={item.code}
                onClick={() => {
                  if (!draft.sicCodes.includes(item.code) && draft.sicCodes.length < 4) {
                    updateDraft({ sicCodes: [...draft.sicCodes, item.code] });
                  }
                }}
                disabled={draft.sicCodes.includes(item.code) || draft.sicCodes.length >= 4}
      ...
      <Card className="bg-muted/50">
        <CardContent className="pt-6">
          <p className="text-sm text-muted-foreground">
            <strong>Note:</strong> You can find the full list of SIC codes on the Companies House website. If
            you're unsure which codes to use, you can search for codes that match your business activities.
          </p>
        </CardContent>
      </Card>
    </div>
