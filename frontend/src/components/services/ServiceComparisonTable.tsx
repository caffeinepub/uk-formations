        ...
        <div className="grid grid-cols-4 gap-6 min-w-[900px]">
          ...
          <div className="space-y-4">
            ...
            {packages[0].features.map((feature, index) => (
              <div key={index} className="h-12 flex items-center font-medium text-sm">
                {feature.name}
              </div>
            ))}
          ...
          {/* Package Columns */}
          {packages.map((pkg) => (
            <Card key={pkg.id} className={`border-2 ${pkg.popular ? 'border-primary shadow-lg' : ''}`}>
              <CardHeader className="text-center pb-8 h-48">
                ...
                <p className="text-sm text-muted-foreground">+ Companies House fee</p>
              </CardHeader>
                ...
                {pkg.features.map((feature, index) => (
                  <div key={index} className="h-12 flex items-center justify-center">
                    {feature.included === true ? (
                      ...
                      <span className="text-sm font-medium">{feature.included}</span>
                    )}
                  </div>
                ))}
